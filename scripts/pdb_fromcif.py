...
def convert_cif_to_pdb(
    ...
    ats_dict = cif_dict[pdb_id]["_atom_site"]

    pdb_lines = []

    for atomidx in range(len(ats_dict["group_PDB"])):

        atom_keyword = ats_dict["group_PDB"][atomidx]
        atom_id = int(ats_dict["id"][atomidx])

        atom_name = ats_dict["label_atom_id"][atomidx]
        resname = ats_dict["label_comp_id"][atomidx]

        chain = ats_dict["auth_asym_id"][atomidx]
        resid = int(ats_dict["label_seq_id"][atomidx])

        x = float(ats_dict["Cartn_x"][atomidx])
        y = float(ats_dict["Cartn_y"][atomidx])
        z = float(ats_dict["Cartn_z"][atomidx])

        occ = float(ats_dict["occupancy"][atomidx])
        bfactor = float(ats_dict["B_iso_or_equiv"][atomidx])

        element = ats_dict["type_symbol"][atomidx].strip()

        segid = chain

        alt_id = " "

        # --- strict PDB formatting ---
        new_line = (
            f"{atom_keyword:<6}{atom_id:>5} "
            f"{atom_name:^4}{alt_id}"
            f"{resname:>3} {chain}{resid:>4}    "
            f"{x:8.3f}{y:8.3f}{z:8.3f}"
            f"{occ:6.2f}{bfactor:6.2f}      "
            f"{segid:<4}"  
            f"{element:>2}"
            f"{os.linesep}"
        )
    ...
    with open(pdb_fname, "w") as wfile:
        wfile.writelines(pdb_lines)


def main(input_cif_fpath: str) -> None:
    cif_fname = Path(input_cif_fpath)
    # name the pdb file as the cif but with the pdb extension
    output_pdb = cif_fname.with_suffix(".pdb")
    print(f"writing pdb file to {output_pdb}")
    convert_cif_to_pdb(cif_fname,output_pdb)
